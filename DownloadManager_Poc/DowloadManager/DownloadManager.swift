...
@objc public protocol DownloadManagerDelegate: class {
    /**A delegate method called each time whenever any download task's progress is updated
     */
    @objc func downloadRequestDidUpdateProgress(_ downloadModel: DownloadModel, index: Int)
    /**A delegate method called when interrupted tasks are repopulated
     */
    @objc func downloadRequestDidPopulatedInterruptedTasks(_ downloadModel: [DownloadModel])
    /**A delegate method called each time whenever new download task is start downloading
     */
    @objc optional func downloadRequestStarted(_ downloadModel: DownloadModel, index: Int)
     ...
     */
    @objc optional func downloadRequestDidPaused(_ downloadModel: DownloadModel, index: Int)
     ...
     */
    @objc optional func downloadRequestDidResumed(_ downloadModel: DownloadModel, index: Int)
    /**A delegate method called each time whenever any download task is resumed. If task is already downloading the action will be ignored
     */
    @objc optional func downloadRequestDidRetry(_ downloadModel: DownloadModel, index: Int)
    /**A delegate method called each time whenever any download task is cancelled by the user
     */
    @objc optional func downloadRequestCanceled(_ downloadModel: DownloadModel, index: Int)
    /**A delegate method called each time whenever any download task is finished successfully
     */
    @objc optional func downloadRequestFinished(_ downloadModel: DownloadModel, index: Int)
    /**A delegate method called each time whenever any download task is failed due to any reason
     */
    @objc optional func downloadRequestDidFailedWithError(_ error: NSError, downloadModel: DownloadModel, index: Int)
     ...
     */
    @objc optional func downloadRequestDestinationDoestNotExists(_ downloadModel: DownloadModel, index: Int, location: URL)
...
open class DownloadManager: NSObject {
    ...
    fileprivate weak var delegate: DownloadManagerDelegate?
    
    open var downloadingArray: [DownloadModel] = []
    
    public convenience init(session sessionIdentifier: String, delegate: DownloadManagerDelegate, sessionConfiguration: URLSessionConfiguration? = nil, completion: (() -> Void)? = nil) {
        self.init()
        self.delegate = delegate
        self.sessionManager = backgroundSession(identifier: sessionIdentifier, configuration: sessionConfiguration)
...
extension DownloadManager {
    ...
    fileprivate func populateOtherDownloadTasks() {
        ...
        for downloadTask in downloadTasks {
            ...
            let downloadModel = DownloadModel.init(fileName: fileName, fileURL: fileURL, destinationPath: destinationPath)
        ...
        for (index, downloadModel) in self.downloadingArray.enumerated() {
            if downloadTask.isEqual(downloadModel.task) {
                DispatchQueue.main.async(execute: { () -> Void in
                    ...
                    let totalFileSize = Utility.calculateFileSizeInUnit(totalBytesExpectedToWrite)
                    let totalFileSizeUnit = Utility.calculateUnit(totalBytesExpectedToWrite)
                    
                    let downloadedFileSize = Utility.calculateFileSizeInUnit(totalBytesWritten)
                    let downloadedSizeUnit = Utility.calculateUnit(totalBytesWritten)
                    
                    let speedSize = Utility.calculateFileSizeInUnit(Int64(speed))
                    let speedUnit = Utility.calculateUnit(Int64(speed))
    ...
    public func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        for (index, downloadModel) in downloadingArray.enumerated() {
            if downloadTask.isEqual(downloadModel.task) {
                ...
                let basePath = downloadModel.destinationPath == "" ? Utility.baseFilePath : downloadModel.destinationPath
                ...
                if fileManager.fileExists(atPath: basePath) {
                    ...
                    //Opportunity to handle the folder does-not exists error appropriately.
                    ...
                    if task.isEqual(downloadModel.task) {
                        if err?.code == NSURLErrorCancelled || err == nil {
                            ...
                            if let error = err {
                                ...
                                let error: NSError = NSError(domain: "DownloadManagerDomain", code: 1000, userInfo: [NSLocalizedDescriptionKey : "Unknown error occurred"])
